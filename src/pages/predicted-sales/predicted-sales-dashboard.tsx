...
import './predicted-sales-dashboard.scss';

export default function PredictedSalesDashboard() {
    ...
    useEffect(() => {
        async function fetchData() {
            const data = await getSalesData(dates.endDate, dates.startDate);
...
async function getSalesData(endDate: Date, startDate?: Date) : Promise<SalesData>  {
    const startParam = startDate ? formatDate(startDate) : '';
    const endParam = endDate ? formatDate(endDate) : '';
    const url = `api/orders/${endParam}?startDate=${startParam}`;
    const body = {
